...
/*
$query2 = 'SELECT * FROM #__properties_rates WHERE productid = '. $this->cids;		
		$db->setQuery( $query2 );
		if (!$db->query2())
		{
			JError::raiseError(500, $db->getErrorMsg() );
		...
		$clone_rates = $db->loadObjectList();
		
foreach($clone_rates as $clone_rate){	
$clone_rate->id='';	
$model = $this->getModel('rates');
if ($model->store($clone_rate)) {	
}
}
*/
