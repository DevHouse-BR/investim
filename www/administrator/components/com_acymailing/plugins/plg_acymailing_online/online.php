		...
		foreach($results as $var => $allresults){
			foreach($allresults[0] as $i => $oneTag){
				...
				if($allresults[1][$i] == 'readonline'){
					$link = acymailing::frontendLink('index.php?option=com_acymailing&ctrl=archive&task=view&mailid='.$email->mailid.'&key='.$email->key.$tmplview.$item);
				}elseif($allresults[1][$i] == 'forward'){
					$link = acymailing::frontendLink('index.php?option=com_acymailing&ctrl=archive&task=forward&mailid='.$email->mailid.'&key='.$email->key.$tmplforward.$item);
				}
