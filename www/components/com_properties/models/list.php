...
class PropertiesModelList extends JModel
{
	var $_data;
	var $TableName = null;	
	var $_total = null;
...
$ShowOrderDefault = $params->get( 'ShowOrderDefault' ) ;
	...
	$this->sqlOrder=$ShowOrderDefault;
			...
			$this->_query = ' SELECT p.*,c.name as name_category,cy.name as name_country,s.name as name_state,l.name as name_locality,pf.name as name_profile,pf.logo_image as logo_image_profile, '
		. ' CASE WHEN CHAR_LENGTH(p.alias) THEN CONCAT_WS(":", p.id, p.alias) ELSE p.id END as Pslug,'
		. ' CASE WHEN CHAR_LENGTH(c.alias) THEN CONCAT_WS(":", c.id, c.alias) ELSE c.id END as Cslug,'
		. ' CASE WHEN CHAR_LENGTH(t.alias) THEN CONCAT_WS(":", t.id, t.alias) ELSE t.id END as Tslug, '	
		. ' CASE WHEN CHAR_LENGTH(cy.alias) THEN CONCAT_WS(":", cy.id, cy.alias) ELSE cy.id END as CYslug,'
...
$this->sqlto= ' AND ap.date < "'.JRequest::getVar('to').'"';
}
$this->sqlAvailable = $this->sqlfrom.$this->sqlto.' AND ap.available <> 0 ';
}
...
$this->sqltextoBuscar = ' AND ( p.name LIKE \'%'.JRequest::getVar('textsearch').'%\' OR p.description LIKE \'%'.JRequest::getVar('textsearch').'%\' )' ;}
	...
	$this->sqlOrder=JRequest::getVar('order');
	}else{
	$this->sqlOrder=$ShowOrderDefault;
	}
	
	
$this->_queryS = ' SELECT p.*,t.name as name_type,cy.name as name_country,s.name as name_state,l.name as name_locality,pf.name as name_profile,pf.logo_image as logo_image_profile, '
				. ' CASE WHEN CHAR_LENGTH(p.alias) THEN CONCAT_WS(":", p.id, p.alias) ELSE p.id END as Pslug,'
		. ' CASE WHEN CHAR_LENGTH(c.alias) THEN CONCAT_WS(":", c.id, c.alias) ELSE c.id END as Cslug,'
		. ' CASE WHEN CHAR_LENGTH(cy.alias) THEN CONCAT_WS(":", cy.id, cy.alias) ELSE cy.id END as CYslug,'
