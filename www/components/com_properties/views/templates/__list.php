...
$action='index.php?option=com_properties';
if(JRequest::getVar('view') == 'properties'){$action.='&view=properties';}

if(JRequest::getVar('task') == 'search'){$action.='&task=search';}
...
if($ShowCategoryInList){
	...
	if($ShowAllParentCategory)
		{
		echo '<span>';
			foreach($ParentCategory[$row->id] as $pc)
			{
			echo JText::_($pc->name).' ';
			}
		echo '</span><br />';
		}else{
