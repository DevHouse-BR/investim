				...
				<tr class="<?php echo "sectiontableentry$k".$this->values->suffix; ?>">
					...
					<td>
						...
						<?php if($this->access->frontEndManament){ ?>
							<a href="<?php echo acymailing::completeLink('newsletter&task=edit&mailid='.$row->mailid.'&listid='.$this->list->listid); ?>"><img src="<?php if($row->visible AND $row->published) echo JURI::base(true)."/images/M_images/edit.png"; else echo JURI::base(true)."/images/M_images/edit_unpublished.png"; ?>"/></a>
							<?php if(!empty($row->senddate)){ ?>
							<a class="modal" rel="{handler: 'iframe', size: {x: 800, y: 590}}" href="<?php echo acymailing::completeLink('newsletter&task=stats&mailid='.$row->mailid.'&listid='.$this->list->listid,true); ?>"><img src="<?php echo ACYMAILING_IMAGES; ?>icons/icon-16-stats.png"/></a>
							<?php } ?>
						<?php }?>
					</td>
					<?php if($this->values->show_senddate) { ?>
